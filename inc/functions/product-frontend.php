		...
		foreach ( $product_addon_sections as $section ) {
			...
			if ( ! empty( $addon_items ) ) {
				...
				if ( $display_section ) {
					...
					wp_nonce_field( 'mspa_addon_product_form_action', 'mspa_addon_product_form_nonce' );
					?>
								...
								<tbody>
									...
									foreach ( $addon_items as $item ) {
										if ( ! empty( $item['name'] ) && ! empty( $item['price'] ) ) {
											...
											<tr>
												<td>
													<input type="checkbox" class="addon-checkbox" data-section="<?php echo esc_attr( $section['product_addon_section_name'] ); ?>" data-price="<?php echo esc_attr( $item['price'] ); ?>" name="addon-<?php echo esc_attr( $item['name'] ); ?>">
...
/**
 * Update the product price based on the selected addons.
 *
 * @return void
 */
function mspa_update_product_price() {
	// validate nonce.
	if ( ! isset( $_POST['mspa_addon_product_form_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['mspa_addon_product_form_nonce'] ) ), 'mspa_addon_product_form_action' ) ) {
		return;
	}

	if ( isset( $_POST['selected_addons'] ) ) {
		$selected_addons = sanitize_text_field( wp_unslash( $_POST['selected_addons'] ) );
...
function mspa_add_selected_addons_to_cart_item_data( $cart_item_data ) {
	if ( ! isset( $_POST['mspa_addon_product_form_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['mspa_addon_product_form_nonce'] ) ), 'mspa_addon_product_form_action' ) ) {
		return;
	}
	...
	if ( isset( $_POST['selected_addons'] ) ) {
		$selected_addons          = sanitize_text_field( wp_unslash( $_POST['selected_addons'] ) );
		$cart_item_data['addons'] = json_decode( $selected_addons, true );
	}
...
function mspa_add_selected_addons_to_order_item_meta( $item, $cart_item_key, $values ) {
...
function mspa_display_selected_addons_in_order( $item_id, $item ) {
