=== Simple Product Addons for WooCommerce ===
Contributors: mswebarts, msshohan
Tags:  woocommerce, product addon, extra fields
Requires at least: 5.5
Tested up to: 6.2
Requires PHP: 7.4
Stable tag: 1.1.1
License: GPLv2 or later
License URI: https://www.gnu.org/licenses/gpl-2.0.html

Let your customers choose product addons while adding product to cart in your WooCommerce website.

== Description ==

Simple Product Addons for WooCommerce is a lightweight plugin that lets you create unlimited amount of product addons in any product. You can divide the addons based on sections.

Simply install and activate the plugin, setup the sections from MS Web Arts -> Product Addons. Then go to your product edit page in WooCommerce and configure as many fields as you want for each section in the Product Addons tab.

== Installation ==

This section describes how to install the plugin and get it working.


**Method One**

1. Go to Plugins -> Add new from your admin panel
2. Search for "Simple Product Addons"
3. Install and activate the plugin

**Method Two**

1. Download the plugin from WordPress.org plugin page
2. Go to your wordpress admin dashboard -> Plugins -> Add New
3. Click on the "Upload Plugin" button
4. Upload the downloaded plugin zip file
5. Then activate it.

Once the plugin is activated, go to MS Web Arts -> Product Addons from the admin menu. You can control all the settings of the plugin from this page.

Pro Tip: If you see any PHP warning in your site's frontend, just go to MS Web Arts -> Product Addons and click on the Save Settings button.

**Method Three**

1. Download the plugin from WordPress.org plugin page
2. Connect to your hosting with an FTP account or File Manager in your Hosting account
3. Go to the WordPress installation directory -> wp-content -> plugins
4. Upload the downloaded plugin's uncompressed folder ( if using FTP ) or zip file ( if Hosting cPanel ). You must extract the file from hosting cPanel if you upload zip file
5. Then go to WordPress admin dashboard -> Plugins -> Installed Plugins -> Then activate Product Addons

Hope the installation process was as smooth as the plugin works!

